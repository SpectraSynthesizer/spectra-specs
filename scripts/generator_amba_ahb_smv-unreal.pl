...
# w_guar_fairness: The same as AMBA but with one additional 
# player 2 fairness conditions.
###############################################
...
if(! defined($ARGV[0])) {
    print "Usage: ./ahb_spec_generator.pl <num_of_masters> <unreal option>\n";
	print "\t<unreal option>: w_guar_fairness, w_guar_trans, or wo_ass_fairness\n";
    exit;
}
...
my $unreal_opt = $ARGV[1];
if(!($unreal_opt eq "w_guar_fairness" or $unreal_opt eq "w_guar_trans" or $unreal_opt eq "wo_ass_fairness")) {
	print "Usage: ./ahb_spec_generator.pl <num_of_masters> <unreal option>\n";
	print "\t<unreal option>: w_guar_fairness, w_guar_trans, or wo_ass_fairness\n";
    exit;
}
...
for(my $i = 0; $i < $num_masters; $i++) {
	...
	if(!($unreal_opt eq "wo_ass_fairness") or ($num_masters != 1)) {
...
#Removed assumption to make the spec unrealizable
if(!($unreal_opt eq "wo_ass_fairness")) {
...
#Additional guarantee to make the spec unrealizable
if($unreal_opt eq "w_guar_trans") {
	$sys_transitions .= "TRANS !next(stateA1_0=0 & hburst0=1);\n";
}
...
# SYS_FAIRNESS
###############################################
#Guarantee for unrealizability
if($unreal_opt eq "w_guar_fairness") {
	$sys_fairness .= "(stateG2_0=1),\n";
}
