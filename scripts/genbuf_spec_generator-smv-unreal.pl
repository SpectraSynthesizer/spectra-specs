...
if(! defined($ARGV[0])) {
    print "Usage: ./genbuf_generator.pl <num_of_senders> <prefix> <unreal option>\n";
...
#Additional fiarness guarantee to make the spec unrealizable
if($unreal_opt eq "w_guar_fairness") {
	$sys_fairness .= "DEQ=1,\n";
}

#Additional guarantee to make the specification unrealizable
if($unreal_opt eq "w_guar_trans") {
	$sys_transitions .= "TRANS ! next(BtoR_REQ0=0 & BtoR_REQ1=0);\n";
}
